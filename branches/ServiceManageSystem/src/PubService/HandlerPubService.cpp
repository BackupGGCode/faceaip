/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
/*
 * HandlerPubService.cpp - _explain_
 *
 * Copyright (C) 2011 ezlibs.com, All Rights Reserved.
 *
 * $Id: HandlerPubService.cpp 5884 2013-01-29 03:21:10Z WuJunjie $
 *
 *  Explain:
 *     -explain-
 *
...
#define SIZE 5000
void CHandlerPubService::tprintf(TcpSocket *p,const char *format, ...)
{
    va_list ap;
    size_t n;
    char tmp[SIZE];
...
void CHandlerPubService::List(TcpSocket *sendto)
{
    int ii=0;
    tprintf(sendto, "HandlerPubService Socket List\n");
    tprintf(sendto, "---------------------------------------------------\n");
...
void CHandlerPubService::Update()
{}

Socket * CHandlerPubService::GetSocketByName(std::string strServerName)
{
    //int ii=0;
...
Socket * CHandlerPubService::GetSocketBySessionID(std::string strSessionID)
{
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++)
    {
        Socket *p = (*it).second;
        CHttpServerPubService *p3 = dynamic_cast<CHttpServerPubService *>(p);
...
//int CHandlerPubService::newSession(const CSession &newss)
//{
//    DBG_CODE(
//        __fline;
...
// ¼øÈ¨
int CHandlerPubService::AuthSession(std::string strSession, int iAccessId)
{
