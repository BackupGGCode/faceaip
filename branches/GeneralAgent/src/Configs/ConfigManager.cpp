...
int CConfigManager::SetDefaultConfig(int iConfigType)
{
    ...
    switch(iConfigType)
    {
        ...
        case CFG_IDX_MY_WEBSERVICE:
        {
            iRet |= recallConfig(CFG_NAME_MY_WEBSERVICE);
...
void CConfigManager::initialize(std::string mfile, std::string sfile)
{
 ...
 /*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
   // 添加通用配置
    ...
    m_mapTranslate.insert(valueType(CFG_NAME_MY_WEBSERVICE, CFG_IDX_MY_WEBSERVICE));
    setupConfig(CFG_NAME_MY_WEBSERVICE, m_configWebService);
/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
}
