...
template<> void exchangeTable<CONFIG_GENERAL>(CConfigExchange& xchg, CConfigTable& table, CONFIG_GENERAL& config, int index, int app)
{
	...
	xchg.exchange(table,"Version", config.strVersion, "v 1.1.0");

	xchg.exchange(table,"LogFilePath", config.strLogFilePath, "./logs");
	xchg.exchange(table,"LogFileName", config.strLogFileName, "tshome-server.log");
	xchg.exchange(table,"maxLogSize", config.maxFileSize,  0, 50*1024*1024 ,5*1024*1024);
	xchg.exchange(table,"maxLogFile", config.maxBackupIndex,  0, 10 ,5);
