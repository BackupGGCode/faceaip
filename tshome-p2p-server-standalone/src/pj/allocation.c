...
#ifdef TSHOME
static pj_status_t stun_on_rx_response(pj_stun_session *sess,
				      const pj_uint8_t *pkt,
				      unsigned pkt_len,
				      const pj_stun_msg *msg,
				      void *token,
...
static void restore_allocation(pj_turn_allocation * allocation);
...
static pj_status_t init_cred(pj_turn_allocation *alloc, const pj_stun_msg *req)
{
    ...
    pj_strdup_with_null(alloc->pool, &alloc->cred.data.static_cred.realm, &realm->value);
    pj_strdup_with_null(alloc->pool, &alloc->cred.data.static_cred.username, &user->value);
    pj_strdup_with_null(alloc->pool, &alloc->cred.data.static_cred.nonce, &nonce->value);
...
#ifdef TSHOME
/*
 * Vefify the credential for the allocation. 
 * If the user or password is not consistent to the one we store in the DB,
 * we return error. 
 */
static pj_status_t verify_usr_passwd (const pj_stun_msg *req)
{
    const pj_stun_username_attr *user;
    const pj_stun_realm_attr *realm;
    const pj_stun_nonce_attr *nonce;
    ...
    /* To compare if the user and password is consistent to DB's */
    //status = tshome_cmp_usr_passwd (username, passwd);
					      ...
					      pj_turn_allocation **p_alloc)
{
    ...
    sess_cb.on_rx_response = &stun_on_rx_response;
		...
		for(; i < sdps.size(); i++)
		{
			if (srv->tables.alloc) {
	            ...
	            while (it) {
	                ...
	                status = init_turn_allocation_key (&dst_key, (const char *)sdps[i].addr, sdps[i].port);
	                if (status != PJ_SUCCESS) {
	                    break;
	                }
...
pj_turn_allocation* pj_turn_allocation_find_by_sdp(pj_turn_srv *srv, user_sdp *sdp)
{
...
static void relay_timeout_cb(pj_timer_heap_t *heap, pj_timer_entry *e)
{
	...
	PJ_LOG(4,(alloc->obj_name, 
		  ...
		  alloc->info));
	LOG4CPLUS_INFO(LOG_ALLOCATION, "Client:"<< alloc->info << " refresh timed-out, shutting down ...");
	char addr[80];
	unsigned addr_len;
	pj_uint16_t port;
	pj_memset(addr, 0, sizeof(addr));
	pj_inet_ntop(PJ_AF_INET, pj_sockaddr_get_addr(&alloc->hkey.clt_addr),
				 addr, sizeof(addr));
...
//__trip;
	...
	PJ_LOG(4,(alloc->obj_name, "Client %s destroying..", 
		  alloc->info));
	LOG4CPLUS_INFO(LOG_ALLOCATION, "Client:"<< alloc->info << " destroying ...");
...
#ifdef TSHOME
/* Create and send successful response with dealt result*/
static void send_reply_ok_with_result (pj_turn_allocation *alloc,
		          const pj_stun_rx_data *rdata,
                          const pj_uint8_t *result,
                          int size)
{
    pj_status_t status;
    unsigned interval;
    ...
    /* Add the callbacks dealt result to user */
...
//printf ("\nhandle_peer_pkt: beform perm\n");
    ...
    char info[256] = {'\0'};
    pj_sockaddr_print(&alloc->relay.hkey.addr, info, 256, 3);
    //printf ("alloc relay addr:%s\n", info);
    memset (info, 0, 256);
    pj_sockaddr_print(&rel->hkey.addr, info, 256, 3);
    //printf ("relay addr:%s\n", info);
    memset (info, 0, 256);
    pj_sockaddr_print(src_addr, info, 256, 3);
    //printf ("src addr:%s\n", info);

//printf ("\nhandle_peer_pkt: after perm\n");
    /* Send Data Indication or ChannelData, depends on whether
     * this permission is attached to a channel number.
     */
    if (perm->channel != PJ_TURN_INVALID_CHANNEL) {
//printf ("\nhandle_peer_pkt: perm channel:%d\n", perm->channel);
	/* Send ChannelData */
	...
	alloc->transport->sendto(alloc->transport, rel->tp.tx_pkt,
			         ...
			         &alloc->hkey.clt_addr,
			         pj_sockaddr_get_len(&alloc->hkey.clt_addr));
//printf ("\nhandle_peer_pkt: perm channel: after send data\n");
    } else { 
	/* Send Data Indication */
	pj_stun_tx_data *tdata;
	pj_status_t status;

//printf ("\nhandle_peer_pkt: Send Data Indication\n");
	...
	pj_stun_session_send_msg(alloc->sess, NULL, PJ_FALSE, 
				 ...
				 pj_sockaddr_get_len(&alloc->hkey.clt_addr), 
				 tdata);
//printf ("\nhandle_peer_pkt: after Send Data Indication\n");
    }
}
 ...
 * response. This callback was trigger by STUN incoming message
				      ...
				      unsigned src_addr_len)
{
    ...
    if (msg->hdr.type == PJ_STUN_REFRESH_RESPONSE) {
		/* 
		...
		if (data_attr != NULL) {
			...
			resched_timeout (alloc);
			alloc->server->core.handler.handle_response(alloc->server, 
												   (unsigned char *)data_attr->data, 
												   data_attr->length);
		}
    }
...
PJ_DECL(void) pj_turn_allocation_restore(pj_turn_allocation * allocation)
{
	restore_allocation(allocation);
}

static void restore_allocation(pj_turn_allocation * allocation)
{
	int peer_count = pj_hash_count(allocation->peer_table);
	//printf ("\nstun_on_rx_request: peer_count=%d\n", peer_count);
	if(peer_count) {
		pj_hash_iterator_t itbuf, *it, *next;
		pj_turn_permission *perm = NULL ;
		it = pj_hash_first (allocation->peer_table, &itbuf);
		while(it){
			perm = (pj_turn_permission *) pj_hash_this(allocation->peer_table, it);
			next = pj_hash_next (allocation->peer_table, it);
			pj_hash_set (NULL, allocation->peer_table,
						 pj_sockaddr_get_addr(&perm->hkey.peer_addr),
						 pj_sockaddr_get_addr_len(&perm->hkey.peer_addr), 0, NULL);
			it = next;	
		}
	}
	int ch_count = pj_hash_count(allocation->ch_table);
	//printf ("\nstun_on_rx_request: ch_count=%d\n", ch_count);
	if(ch_count) {
		pj_hash_iterator_t itbuf, *it, *next;
		pj_turn_permission *perm = NULL;
		it = pj_hash_first (allocation->ch_table, &itbuf);
		while (it) {
			perm = (pj_turn_permission *) pj_hash_this(allocation->ch_table, it);
			next = pj_hash_next (allocation->ch_table, it);
			pj_hash_set (NULL, allocation->ch_table,
						 &perm->channel,
						 sizeof(perm->channel), 0, NULL);
			it = next;	
		}
	...
	LOG4CPLUS_DEBUG(LOG_TURN, "end restore_allocation " << allocation);
	//peer_count = pj_hash_count(allocation->peer_table);
	//ch_count = pj_hash_count(allocation->ch_table);
	//printf ("\nstun_on_rx_request: peer_count=%d, ch_count=%d\n", peer_count, ch_count);
...
//printf("new alloc p= %p\n", alloc);
	...
	if (lifetime && lifetime->value==0) {
        ...
        data_attr = (pj_stun_data_attr *)
                     pj_stun_msg_find_attr (msg, PJ_STUN_ATTR_DATA, 0);
		if (data_attr != NULL) {
			pj_uint8_t result[4096] = {0};
			unsigned result_len = 0;//= sizeof (result);
			char addr[80];
			unsigned addr_len;
			pj_uint16_t port;
			pj_memset(addr, 0, sizeof(addr));
			pj_inet_ntop(PJ_AF_INET, pj_sockaddr_get_addr(src_addr),
						 addr, sizeof(addr));
			addr_len = strlen(addr) + 1;
			port = pj_sockaddr_get_port(src_addr);
			
			user_sdp sdp;
			LOG4CPLUS_DEBUG(LOG_ALLOCATION, ">>> handle_request:"
				<< alloc->cred.data.static_cred.username.slen << " "
				<< data_attr->length);
			//__trip;
			//printf("alloc->relay.lifetime=%d\n",alloc->relay.lifetime);
			...
			/* Send reply */
			send_reply_ok_with_result (alloc, rdata, result, result_len);
        } else {
			/* Send reply */
			send_reply_ok(alloc, rdata);
        }
 ...
 *                   */
        ...
        unsigned index = 0, total = 1, one = 0;

        while (index < total) {
            peer_attr = (pj_stun_xor_peer_addr_attr*)
                            pj_stun_msg_find_attr2(msg, PJ_STUN_ATTR_XOR_PEER_ADDR, &index, &total);
            //peer_attr = (pj_stun_xor_peer_addr_attr*)
              //          pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_XOR_PEER_ADDR, index);
...
//printf ("\nstun_on_rx_indication: before alloc\n");
    alloc = (pj_turn_allocation*) pj_stun_session_get_user_data(sess);

//printf ("\nstun_on_rx_indication: after alloc\n");
...
//printf ("\nstun_on_rx_indication: PJ_STUN_SEND_INDICATION\n");
...
//printf ("\nstun_on_rx_indication: peer_attr is Not NULL\n");
    /* Get DATA attribute */
    data_attr = (pj_stun_data_attr*)
		pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_DATA, 0);
...
//printf ("\nstun_on_rx_indication: perm is Not NULL\n");
    refresh_permission(perm);
...
//printf ("\nstun_on_rx_indication: data is Not NULL\n");

    char info[256] = {'\0'};
    pj_sockaddr_print(&alloc->relay.hkey.addr, info, 256, 3);
//    printf ("alloc relay addr:%s\n", info);
    memset (info, 0, 256);
    pj_sockaddr_print(&peer_attr->sockaddr, info, 256, 3);
//    printf ("peer addr :%s\n", info);
    /* Relay the data to peer */
    len = data_attr->length;
    int ret = pj_sock_sendto(alloc->relay.tp.sock, data_attr->data, 
		   &len, 0, &peer_attr->sockaddr,
		   pj_sockaddr_get_len(&peer_attr->sockaddr));

    if (ret != PJ_SUCCESS) {
//        printf ("\nstun_on_rx_indication: data is Not NULL, ret = %d\n", ret);
        return ret;
    }
