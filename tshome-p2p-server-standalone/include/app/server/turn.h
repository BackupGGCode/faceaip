...
struct pj_turn_srv
{
	...
	pj_hash_table_t *res;
#ifdef TSHOME
	/** Host users hash table, indexed by homeid,
	 *  and stored a user list for each home. 
	 */
	pj_hash_table_t *hosts;

	/** Mobile users hash table, indexed by homeid,
	 *  and stored a user list for each home. 
	 */
	pj_hash_table_t *mobiles;

	/** 
	 * session id mapping to home id
	 */
	pj_hash_table_t *stoh;

	/** a temporary user database , indexed by userName,
	 *  and stored each user which selected from database. 
	 */
	pj_hash_table_t *tmpdb;
#endif
    } tables;
...
/**
 *shutdown and destroy all allocations of the user, called by stun_on_rx_request() when user logout.
 *added by sunzq
 */
PJ_DECL(void) pj_turn_allocation_destroy_by_user(pj_turn_srv *srv, ts_user *user);

/**
 *find an allocation by SDP
 *added by sunzq
 */
