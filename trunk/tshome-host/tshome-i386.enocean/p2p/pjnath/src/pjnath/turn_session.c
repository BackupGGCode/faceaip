...
#ifdef TSHOME
    stun_cb.on_rx_request = &stun_on_rx_request;
    stun_cb.on_rx_response = &stun_on_rx_response;
#endif
...
#ifdef TSHOME
void send_data_to_srv (pj_turn_session *sess, int lifetime, const pj_uint8_t *buf, int size)
{
    pj_stun_tx_data *tdata;
    pj_status_t status;
    ...
    /* Create a bare REFRESH request with SDP or CMD */
    status = pj_stun_session_create_req(sess->stun, PJ_STUN_REFRESH_REQUEST,
					PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS)
	goto on_error;
...
static void send_reply_ok_with_result (pj_turn_session *sess,
                          const pj_stun_rx_data *rdata,
			  ...
			  unsigned dst_addr_len)
{
    ...
    status = pj_stun_session_create_res(sess->stun, rdata, 0, NULL, &tdata);
    if (status != PJ_SUCCESS) {
    ...
    status = pj_stun_session_send_msg(sess->stun, NULL, PJ_TRUE,
                                      PJ_FALSE, dst_addr,
                                      dst_addr_len,
                                      tdata);
    if (status != PJ_SUCCESS) {
...
/*
 * Notification from STUN session on incoming STUN Request 
 * message.
 */
static pj_status_t stun_on_rx_request(pj_stun_session *stun,
					 const pj_uint8_t *pkt,
					 unsigned pkt_len,
					 const pj_stun_rx_data *rdata,
					 void *token,
                                         ...
                                         unsigned *result_len)
{
    ...
    /* Expecting REFRESH Request only */
    if (msg->hdr.type != PJ_STUN_REFRESH_REQUEST) {
	PJ_LOG(4,(sess->obj_name, "Unexpected STUN %s request",
		  pj_stun_get_method_name(msg->hdr.type)));
	return PJ_EINVALIDOP;
    }
    ...
    /* Must have DATA attributes */
    if (!data_attr) {
	PJ_LOG(4,(sess->obj_name, 
		  "Received Refresh Request with missing attributes"));
	return PJ_EINVALIDOP;
    }

    // Call user callback function to deal with the CMD from SRV
    // callback_function ();
    //Send OK Reply
    /*char result[1000];
    char *str = "RESPONSE_FROM_CLIENT";
    int size = strlen(str) + 1;
    memcpy (result, str, size);
    rdata = (pj_stun_rx_data *)token;    
    //send_reply_ok_with_result (sess, rdata, result, size, src_addr, src_addr_len); 
*/
    /* Notify application */
    if (sess->cb.on_rx_data) {
        char data_buf[1000];
        pj_memset (data_buf, 0, 1000);
        unsigned data_len;

        /* Copy the data_attr data to data buffer, when the callback returned, 
         * the data buffer contains the application dealt result, the format is:
         * RESULT_LEN:RESULT, Send the result as response to SRV.
         */
        pj_memcpy (data_buf, data_attr->data, data_attr->length);
	(*sess->cb.on_rx_data)(sess, data_buf, data_attr->length, 
			       src_addr, src_addr_len,
                               result, result_len);

        /* Parse the result */
     //   char *ptr = pj_strchr (data_buf, ':');
     //   if (NULL == ptr) {
     //       printf ("The Result format is invalid.\n");
    //        return !PJ_SUCCESS;
     //   } else {
       //     *ptr = '\0';
         //   ptr++;
           // data_len = atoi(data_buf);
           // send_reply_ok_with_result (sess, rdata, ptr, data_len, src_addr, src_addr_len);
     //       send_reply_ok_with_result (sess, rdata, data_attr->data, data_attr->length, src_addr, src_addr_len);
        //}
    }
...
/*
 * Notification from STUN session on incoming STUN Response 
 * message.
 */
static pj_status_t stun_on_rx_response(pj_stun_session *stun,
					 const pj_uint8_t *pkt,
					 unsigned pkt_len,
					 const pj_stun_msg *msg,
					 void *token,
					 ...
					 unsigned src_addr_len)
{
    ...
    if (!data_attr) {
	PJ_LOG(4,(sess->obj_name, 
		  "Received Refresh Response with missing attributes"));
	return PJ_EINVALIDOP;
    }
...
*/
    ...
    if (sess->cb.on_rx_data) {
	(*sess->cb.on_rx_data)(sess, data_attr->data, data_attr->length, 
			       src_addr, src_addr_len, NULL, NULL);
    }
