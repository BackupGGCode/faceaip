...
#ifdef TSHOME
    unsigned		     peer_id;	/**< The Number ID of this component*/
#endif
...
#else
static pj_status_t create_comp(pj_ice_strans *ice_st,
                               unsigned comp_id,
                               unsigned inner_mode)
#endif
{
...
#ifdef TSHOME 
        /* Add Server Reflexive Candicate when in TURN Mode */
/*        pj_stun_sock_info stun_sock_info;
        pj_ice_sess_cand *cand = NULL;
        int i;
...
*/
        /* Initialize default candidate to SRFLX candicate */
 /*       comp->default_cand = cand - comp->cand_list;*/
#endif
    }
...
#else
	status = create_comp(ice_st, i+1, inner_mode);
#endif
	if (status != PJ_SUCCESS) {
...
#ifdef TSHOME
PJ_DEF(void) tshome_set_ice_ufrag(pj_ice_strans *ice_st, pj_str_t *ufrag)
{
    pj_strcpy(&(ice_st->ice->rx_ufrag), ufrag);
}

PJ_DEF(void) tshome_set_ice_upass(pj_ice_strans *ice_st, pj_str_t *upass)
{
    pj_strcpy(&(ice_st->ice->rx_pass), upass);
}
#endif
...
#ifdef TSHOME
void tshome_set_peer_id (pj_ice_strans * icest, unsigned peer_id)
{
    icest->peer_id = peer_id;
}

unsigned tshome_get_peer_id (pj_ice_strans * icest)
{
    return icest->peer_id;
}


/*
 * Application wants to send outgoing packet to server.
 */
PJ_DEF(pj_status_t) pj_ice_strans_sendto_srv( pj_ice_strans *ice_st,
					  const void *data,
					  pj_size_t data_len)
{
    pj_ssize_t pkt_size;
...
//#else
//	if ((comp->cand_list[def_cand].type == PJ_ICE_CAND_TYPE_RELAYED) ||
//             (comp->cand_list[def_cand].type == PJ_ICE_CAND_TYPE_SRFLX)) {
//#endif
	    enum {
		msg_disable_ind = 0xFFFF &
				  ~(PJ_STUN_SESS_LOG_TX_IND|
	    ...
	    status = pj_turn_sock_sendto_srv (comp->turn_sock, (const pj_uint8_t*)data, data_len);
	    return (status==PJ_SUCCESS||status==PJ_EPENDING) ? 
		    PJ_SUCCESS : status;
	} 
    } else
...
//        pj_lock_acquire(comp->ice_st->init_lock);
        ...
        /* Find srflx candidate in the component */
/*        for (i=0; i<comp->cand_cnt; ++i) {
            if (comp->cand_list[i].type == PJ_ICE_CAND_TYPE_SRFLX) {
                cand = &comp->cand_list[i];
                break;
...
*/
        /* Update candidate */
/*        pj_sockaddr_cp(&cand->addr, &rel_info.mapped_addr);
        pj_ice_calc_foundation(comp->ice_st->pool, &cand->foundation,
                               PJ_ICE_CAND_TYPE_SRFLX,
                               &rel_info.mapped_addr);
        cand->status = PJ_SUCCESS;
        comp->default_cand = cand - comp->cand_list;
*/
//#endif
