				       ...
				       pj_turn_srv **p_srv)
{
    ...
    /* Create hash tables */
    // res on turn.h L437
    srv->tables.alloc = pj_hash_create(pool,  __cfgP2P.getConfig().maxClient);
    srv->tables.res = pj_hash_create(pool, __cfgP2P.getConfig().maxClient);
    srv->tables.hosts = pj_hash_create(pool, __cfgP2P.getConfig().maxClient);
    srv->tables.mobiles = pj_hash_create(pool, __cfgP2P.getConfig().maxClient);
    srv->tables.stoh = pj_hash_create(pool, __cfgP2P.getConfig().maxClient);
    srv->tables.tmpdb = pj_hash_create(pool, __cfgP2P.getConfig().maxClient);

    /* Init ports settings */
    srv->ports.min_udp = srv->ports.next_udp = __cfgP2P.getConfig().minUdpPort;
    srv->ports.max_udp = __cfgP2P.getConfig().maxUdpPort;
    srv->ports.min_tcp = srv->ports.next_tcp = __cfgP2P.getConfig().minUdpPort; // no use now wujj 2012-8-23 9:38:01
    srv->ports.max_tcp = __cfgP2P.getConfig().maxUdpPort;// no use now wujj 2012-8-23 9:38:01
    ...
    //init tshome protocol handler
    status = tsp_handler_init(srv);
    if (status != PJ_SUCCESS) {
	goto on_error;
    }
...
static void srv_handle_events(pj_turn_srv *srv, const pj_time_val *max_timeout)
{
    if (!srv || !max_timeout)
    {
        LOG4CPLUS_ERROR(LOG_IPC, "srv or max_timeout not valid.");
    }
...
PJ_DEF(pj_status_t) pj_turn_srv_destroy(pj_turn_srv *srv)
{
	...
	srv->tables.res = NULL;
	srv->tables.hosts = NULL;
	srv->tables.mobiles = NULL;
	srv->tables.stoh = NULL;
	srv->tables.tmpdb = NULL;
    }
