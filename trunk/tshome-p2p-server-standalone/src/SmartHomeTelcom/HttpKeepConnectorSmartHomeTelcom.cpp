...
#define FIND_BY_ID_SQL "SELECT	\
							t_floor.floorName,t_room.roomName, t_device.id, \
							t_device.deviceName,t_devicetype.deviceTypeName,t_jstelcom_user.userid \
						FROM \
							t_device,t_room,t_floor,t_home,t_devicetype,t_jstelcom_user \
						WHERE \
...
DBG(
    ...
    switch(m_iCommunicationStatus)
    {
        ...
        case EnCommunicationStatus_reportDeviceStateListing:
        {
            ...
            {
                std::cout << "! reportDeviceStateList Failed, Reason:"
                << __op.reason
                << std::endl
        ...
        case EnCommunicationStatus_reportAlarming:
        {
            ...
            {
                //std::cout << "EnCommunicationStatus_reportCtrlDeviceStateed, SessionId:" << m_strSessionId << std::endl;
                std::cout << "reportAlarm Succeeded, Reason:" \
                << __op.reason \
                << std::endl \
            ...
            {
                std::cout << "! reportAlarm Failed, Reason:"
                << __op.reason
                << std::endl
...
int CHttpKeepConnectorSmartHomeTelcom::reportCtrlDeviceState()
{
    ...
    Msg.State 		= "1";
    Msg.StateTime 	= get_date_time_string();
...
int CHttpKeepConnectorSmartHomeTelcom::reportDeviceStateList()
{
    CConfigSmartHomeTelcom __cfg;
    __cfg.update();
    //__fline;
    //printf("CHttpKeepConnectorSmartHomeTelcom::reportDeviceStateList\n");
    ...
    __operate.value_time       = get_date_time_string();
...
int CHttpKeepConnectorSmartHomeTelcom::reportAlarm()
{
    ...
    _operate.value_time       = get_date_time_string();//"2012-5-17 10:13:52";
...
int loadAllDevices(std::vector<CAgentMsg_initDeviceConfigParam> &deviceVec)
{
    ...
    res = mysql_query(mysql, FIND_ALL_HOME_DEVICE); 
    if(res)
    {
        LOG4CPLUS_ERROR(LOG_DB, "query error");
    }
    else
    {
        ...
        if(res_ptr)
        {
            ...
            cout
            <<"floor\t|room\t|deviceSN\t|deviceName\t|deviceType\t|userName\t|deviceId\t|\n";
            cout << "result counter = " << mysql_num_rows(res_ptr) << "\n";
            while((sqlrow = mysql_fetch_row(res_ptr)))
            {
                printf("%s,%s,%s,%s,%s,%s,%s\n", sqlrow[0],sqlrow[1],sqlrow[2],sqlrow[3],sqlrow[4],sqlrow[5],sqlrow[6]);
                CAgentMsg_initDeviceConfigParam __device;
                CAgentMsg_initDeviceConfigParam_Seat_deploy __deploy;
                CAgentMsg_initDeviceConfigParam_Seat_deploy_Operate __operate;
                ...
                __operate.operate_type    = "0"; //开关
                __operate.operate_value   = "2"; // 关
                __operate.operate_explain = sqlrow[3];
                ezGBKToUTF8(__operate.operate_explain);
                // 物品
                __deploy.deploy_name = sqlrow[3];
                ezGBKToUTF8(__deploy.deploy_name);
                ...
                __device.sessionId 	= "";
                __device.productID 	= "";
                __device.userID 		= sqlrow[5];
                __device.deviceID 	= sqlrow[6]; //
