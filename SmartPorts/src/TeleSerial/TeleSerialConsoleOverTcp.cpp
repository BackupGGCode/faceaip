...
void CTeleSerialConsoleOverTcp::OnAccept()
{
    time_t timep;
    struct tm *p;
    time (&timep);
    ...
    LOG4CPLUS_INFO(LOG_SOLAR, "CTeleSerialConsoleOverTcp::OnAccept:" << GetRemoteAddress() << ":" << GetRemotePort());
}

CTeleSerialConsoleOverTcp::~CTeleSerialConsoleOverTcp()
{
    LOG4CPLUS_INFO(LOG_SOLAR, "CTeleSerialConsoleOverTcp::Destroy:" << GetRemoteAddress() << ":" << GetRemotePort());
}

#define SIZE 5000
void CTeleSerialConsoleOverTcp::tprintf(const char *format, ...)
{
    va_list ap;
    size_t n;
    char tmp[SIZE];
...
void CTeleSerialConsoleOverTcp::OnLine(const std::string& InputLine)
{
    ...
    else if (cmd == "status" || cmd == "st" || cmd == "d")
    {
        ...
        time_t ttStart =  g_TeleSerial.GetStartTime();
        unsigned int ttLast = g_TeleSerial.GetRunPeriod();
#else
        time_t ttStart =  0;//g_TeleSerial.GetStartTime();
        unsigned int ttLast = 0;//g_TeleSerial.GetRunPeriod();
#endif // ATTATCH_FUNC_OF_GENERAL_AGENT

        tprintf(" Sys Status:\n%s\n", TOKEN_ConsoleOverTcp);
        tprintf("\t%-8s%s\n", "Ver:", /*g_TeleSerial.GetVersion().c_str()*/"hh");
        ...
        tprintf("\n Socket Status:\n%s\n", TOKEN_ConsoleOverTcp);
        static_cast<CTeleSerialHandler&>(Handler()).List(this);
    ...
    else if (cmd == "sd" || cmd == "s")
    {
        ...
        g_TeleSerial.Send2Socket(strName, strSend.c_str(), strSend.size());
...
void CTeleSerialConsoleOverTcp::Update()
{
...
void CTeleSerialConsoleOverTcp::DumpPjturnStatus(/*pj_turn_srv *srv*/)
{
    tprintf("DumpPjturnStatus\n");
    tprintf("------------------------------------------------------------------------------\n");
}
...
void CTeleSerialConsoleOverTcp::Kick(std::string userName)
{}

void CTeleSerialConsoleOverTcp::DumpIPCWebinfo()
{
    tprintf("DumpIPCWebinfo\n");
    tprintf("------------------------------------------------------------------------------\n");
}

void CTeleSerialConsoleOverTcp::DumpUserInfo(std::string home)
{
...
void CTeleSerialConsoleOverTcp::TestAPNSPush(std::string msg)
{}
