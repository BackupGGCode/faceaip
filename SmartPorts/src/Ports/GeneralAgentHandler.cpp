...
void GeneralAgentHandler::Update()
{
    for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); it++)
    {
        ...
        CGeneralAgentTcpSocketConnector *pConnector = dynamic_cast<CGeneralAgentTcpSocketConnector *>(p0);
        if (pConnector)
        {
            pConnector->Update();
        }
		
        CGeneralConsoleOverTcp *pConsoleOverTcp = dynamic_cast<CGeneralConsoleOverTcp *>(p0);
        if (pConsoleOverTcp)
        {
            pConsoleOverTcp->Update();
        }
