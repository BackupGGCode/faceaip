...
#ifdef TSHOME
/* Send response with data attr */
static pj_status_t send_response_with_data (pj_stun_session *sess, void *token,
				 pj_pool_t *pool, pj_stun_msg *response,
				 const pj_stun_req_cred_info *auth_info,
				 pj_bool_t retransmission,
				 const pj_sockaddr_t *addr, unsigned addr_len,
                                 const pj_uint8_t *result, int len)
{
    pj_uint8_t *out_pkt;
    pj_size_t out_max_len, out_len;
    pj_status_t status;
    ...
    /* Add data attr */
    status = pj_stun_msg_add_binary_attr (pool, response,
                                          PJ_STUN_ATTR_DATA,
                                          (const pj_uint8_t*)result, len);
    PJ_ASSERT_RETURN(status==PJ_SUCCESS, status);
					...
					unsigned src_addr_len)
{
    ...
    if (msg->hdr.type == PJ_STUN_REFRESH_RESPONSE) {
        pj_stun_data_attr *data_attr = pj_stun_msg_find_attr (msg,
                                           PJ_STUN_ATTR_DATA, 0);
        if (data_attr != NULL) {
           //TODO: call dealt with fuction
           //username: the data which come from
           //result: the SRV response data
           //len: the data size
           //deal_with_data (char *username, const pj_uint8_t *result, int len) 
            /* Distribute to handler */
            if (sess->cb.on_rx_response) {
	        return (*sess->cb.on_rx_response)(sess, pkt, pkt_len, msg,
...
#else
        pj_uint8_t result[1000];
        pj_memset (result, 0, 1000);
	...
	status = (*sess->cb.on_rx_request)(sess, in_pkt, in_pkt_len, &rdata,
        ...
        if ( status == PJ_SUCCESS ) {
           ...
           if (msg->hdr.type == PJ_STUN_BINDING_REQUEST)
            ...
            status = pj_stun_msg_create_response(tmp_pool, msg, 0,
                                             NULL, &response);
            if (status == PJ_SUCCESS && response) {
                status = send_response_with_data (sess, token, tmp_pool, response,
                                   &rdata.info, PJ_FALSE, src_addr, src_addr_len,
                                   result, result_len);
            }
        }
