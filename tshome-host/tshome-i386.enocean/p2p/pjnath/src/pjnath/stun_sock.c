/* $Id: stun_sock.c 3765 2011-09-22 01:31:21Z bennylp $ */
/* 
...
#ifdef TSHOME
        if (inner_mode) {
            pj_sockaddr_set_port(&bound_addr, (INNER_MODE_PORT+inner_mode));
            status = pj_sock_setsockopt(stun_sock->sock_fd, SOL_SOCKET, SO_REUSEADDR,
	        			&val, sizeof(val));
            if (status != PJ_SUCCESS)
	        goto on_error;
        }
